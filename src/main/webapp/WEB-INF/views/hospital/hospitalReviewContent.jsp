...
<head>
	<meta charset="UTF-8">
	<title>hospitalReviwContent.jsp</title>
	...
	<script>
    	...
    	function hospitalReviewDelete() {
    		let ans = confirm("해당 병원 리뷰를 삭제 하시겠습니까?");
        	if(ans) location.href = "hospitalReviewDelete?idx=${vo.idx}&placeIdx=${cafeVO.idx}";
		}
	...
	<div class="inner">
		<div class="cafeInfoSec">
			...
			<section class="infoSec">
				...
				<c:if test="${empty sWishPlace}"><p>방문자후기 0 • 찜 <button onclick="wishPickBtn(${cafeVO.idx})"><img src="${ctp}/resources/images/icon/love-3.png" style="width:30px;"/></button></p></c:if>
				<c:if test="${!empty sWishPlace}"><p>방문자후기 0 • 찜 <button onclick="wishPickBtn(${cafeVO.idx})"><img src="${ctp}/resources/images/icon/love.png" style="width:30px;"/></button></p></c:if>
				<hr/>
				...
				<input type="button" value="돌아가기" onclick="location.href='hospitalReviewList?idx=${cafeVO.idx}';" class="backBtn" />
			</section>
		</div>
			...
			<section>
				<p>
					...
					<c:if test="${sMid == vo.mid}">
						<button class="friend" onclick="location.href='hospitalReviewUpdate?idx=${vo.idx}&placeIdx=${cafeVO.idx}';">리뷰수정</button>
						<button class="friend" onclick="hospitalReviewDelete()">리뷰삭제</button>
					</c:if>
				</p>
				<hr/>
