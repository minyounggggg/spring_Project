...
<head>
	<meta charset="UTF-8">
	<title>hospitalReviwList.jsp</title>
...
<jsp:include page="/WEB-INF/views/include/nav.jsp" />
	<div class="inner">
		<div class="cafeInfoSec">
			...
			<section class="infoSec">
				...
				<input type="button" value="돌아가기" onclick="location.href='hospital';" class="backBtn" />
			</section>
		</div>
		<%-- 
		...
		<div class="cafeReviewSec">
			<c:forEach var="vo" items="${vos}" varStatus="st">
				<%-- <c:if test="${vo.idx == null}">아직 병원 리뷰가 없습니다!</c:if> --%>
				<section onclick="location.href='hospitalReviewContent?idx=${vo.idx}&placeIdx=${vo.placeIdx}';" style="cursor: pointer;">
					<p>
			...
			<div class="text-center" style="clear:both;">
				...
				<c:if test="${pageVO.pag > 1 }"><li class="page-item"><a class="page-link text-secondary" href="hospitalReviewList?idx=${cafeVO.idx}&pag=1&pageSize=${pageVO.pageSize}">첫 페이지</a></li></c:if>
				<c:if test="${pageVO.curBlock > 0}"><li class="page-item"><a class="page-link text-secondary" href="hospitalReviewList?idx=${cafeVO.idx}&pag=${(pageVO.curBlock-1) * pageVO.blockSize + 1}&pageSize=${pageVO.pageSize}">이전블록</a></li></c:if>
				<c:forEach var="i" begin="${(pageVO.curBlock * pageVO.blockSize)+1}" end="${(pageVO.curBlock * pageVO.blockSize) + pageVO.blockSize}" varStatus="st">
					<!-- 현재있는 페이지 수만 굵게 표시한다. -->
					<c:if test="${i <= pageVO.totPage && i == pageVO.pag}"><li class="page-item active"><a class="page-link" href="hospitalReviewList?idx=${cafeVO.idx}&pag=${i}&pageSize=${pageVO.pageSize}">${i}</a></li></c:if> 
					<!-- 현재있는 페이지가 아닌 숫자는 일반 표시, 두껍게 하지 않는다. -->
					<c:if test="${i <= pageVO.totPage && i != pageVO.pag}"><li class="page-item"><a class="page-link text-secondary" href="hospitalReviewList?idx=${cafeVO.idx}&pag=${i}&pageSize=${pageVO.pageSize}">${i}</a></li></c:if>
				</c:forEach>
				<c:if test="${pageVO.curBlock < pageVO.lastBlock}"><li class="page-item"><a class="page-link text-secondary" href="hospitalReviewList?idx=${cafeVO.idx}&pag=${(pageVO.curBlock+1) * (pageVO.blockSize+1)}&pageSize=${pageVO.pageSize}">다음블록</a></li></c:if>
				<c:if test="${pageVO.pag < pageVO.totPage}"><li class="page-item"><a class="page-link text-secondary" href="hospitalReviewList?idx=${cafeVO.idx}&pag=${pageVO.totPage}&pageSize=${pageVO.pageSize}">마지막 페이지</a></li></c:if>
				</ul>
			</div>
			<!-- 블록페이지 끝! -->
		</div>
	...
	<div class="insertNav" style="bottom: 100px; right: 90px; position: fixed;">
		<section onclick="location.href='hospitalReviewInsert?placeIdx=${cafeVO.idx}';"><img src="${ctp}/resources/images/icon/insertBtn.png" style="margin-bottom:10px;width:80px;cursor:pointer;"/></section>
